...
public class Blackjack {
	...
	public static void main(String[] args) {
		...
		while (cash > 0) {
			Deck deck = new Deck();// initialize deck, dealer, hands, and set
									// the bet.
			deck.shuffle();
			AceCounter = 0;
			...
			if (hasBlackJack(handvalue) && dealer.hasBlackJack())// check if
																	// both the
																	// user and
																	// dealer
																	// have
			...
			{
				Push();
			} else if (hasBlackJack(handvalue))// check if the user has
												// blackjack.
			{
				...
				cash = cash + bet;
				Win();
			} else if (dealer.hasBlackJack())// check if the dealer has
												// blackjack.
			{
				...
				{
					System.out.println("Would you like to double down?");// allows
																			// the
																			// user
																			// to
																			// double
				...
				System.out.println("Would you like to hit or stand?");// ask if
																		// the
																		// user
																		// will
																		// hit
				...
				while (hitter.equals("hit"))// hits the user as many times as he
											// or she pleases.
				{
					Hit(deck, hand);
					...
					if (handvalue <= 21 && hand.size() == 5)// checks for a five
															// card trick.
					{
						fivecardtrick();
						break;
				...
				{
					int dealerhand = dealer.takeTurn(deck);// takes the turn for
															// the dealer.
					System.out.println("");
					System.out.println("Here is the dealer's hand:");
					dealer.showHand();
					if (dealerhand > 21)// if the dealer busted, user wins.
					{
						...
						int you = 21 - handvalue;// check who is closer to 21
			...
			System.out.println("Would you like to play again?");// ask if the
																// user wants to
																// keep going
		...
		System.out.println("Your cash is: " + cash);// if user doesn't want to
													// play or runs out of cash,
													// either congratulates them
													// on their winnings or lets
													// them know
		if (cash == 0) {
	...
	/*
	 * Adds a card to user's hand and calculates the value of that hand. Aces
